...
for rtid in ROUTES_ACTIVE.keys():
    ...
    if len(pids) > 1:
        ...
        patternSecondary = [Stop(pt['lon'], pt['lat'])
                            for pt in patternSecondaryJSON['pt']]
...
routePatterns['61s-1'] = filterStopsBoundingBox(
    temp61s, -80, -79.96, 40.43, 40.445)
routePatterns['61s-2'] = filterStopsBoundingBox(
    temp61s, -79.94, -79.90, 40.41, 40.44)
routePatterns['54-1'] = filterStopsBoundingBox(
    routePatterns['54-1'], -79.99, -79.94, 40.44, 40.47)
routePatterns['71B-temp'] = filterStopsBoundingBox(
    routePatterns['71B-0'], -79.93, -79.91, 40.45, 40.48)
routePatterns['71B-0'] = [stop for stop in routePatterns['71B-0']
                          if stop not in routePatterns['71B-temp']]


def scaleLatAll(routePatterns, s):
    for id in routePatterns.keys():
        routePatterns[id] = filterStopsBoundingBox(
            routePatterns[id], llon, rlon, blat, ulat)
...
routePatterns['61s-0'] += routePatterns['61s-1']
routePatterns['61s-1'] = routePatterns['61s-2']
del routePatterns['61s-2']

fullRoutes = [routePatterns[id] for id in routePatterns.keys()]
plotPGH(fullRoutes)
print(len(flatten(fullRoutes)))

write_json('data/routePatterns.json', routePatterns)
